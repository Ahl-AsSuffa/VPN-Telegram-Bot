...
Перед публикацией репозитория обязательно проверьте, что наружу не ушли:

- токен бота и строка подключения к PostgreSQL;
- логин и `Пароль №1` от кассы Robokassa;
- публичный ключ Reality и реальные `shortId` — формально это не секреты, но они однозначно привязывают код к вашему боевому серверу;
- ваш личный Telegram ID в `MyTgId`, `adminIds` и в текстах-примерах.

Рекомендуется вынести всю конфигурацию в `appsettings.json` или переменные окружения и добавить файл в `.gitignore`. Если настоящие ключи когда-либо попадали в коммит, замены в текущей версии недостаточно — они остаются в истории git, и токен нужно перевыпустить.
...
Укажите лицензию по своему усмотрению (например, MIT).
